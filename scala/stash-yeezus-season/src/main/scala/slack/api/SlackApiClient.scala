...
class SlackApiClient(token: String) {
  ...
  def getUserProfileInfo(userId: String)(implicit system: ActorSystem): Future[JsValue] = {
    val res = makeApiMethodRequest("users.profile.get", "user" -> userId)
    res
  }
  ...
  def makeApiMethodRequest(apiMethod: String, queryParams: (String,Any)*)(implicit system: ActorSystem): Future[JsValue] = {
